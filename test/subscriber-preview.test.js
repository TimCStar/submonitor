...
import { buildSubscriberPreview, maskSubscriberIdentity, toPublicSubscriberPreview } from "../src/server/subscriber-preview.js";
...
test("subscriber preview paginates active subscriptions and never returns raw identities", async () => {
        ...
        items: [
          {
            ...
            group: { name: "Codex Pro", weekly_limit_usd: 20 },
          },
  ...
  assert.deepEqual(preview.subscribers[0].usage.weekly, { used: 5, limit: 20, percentage: 25 });
...
test("public subscriber preview omits internal group identifiers", () => {
  const preview = toPublicSubscriberPreview({
    groups: [10],
    resetWindows: ["weekly"],
    total: 1,
    truncated: false,
    ...
    subscribers: [{
      identity: { kind: "email", leading: "a", trailing: "e", obscured: true, suffix: "@example.com" },
      groupId: 10,
      groupName: "Codex Pro",
      status: "active",
      expiresAt: null,
      usage: { weekly: { used: 5, limit: 20, percentage: 25 } },
    }],
  });

  assert.equal(preview.groupCount, 1);
  assert.equal(Object.hasOwn(preview, "groups"), false);
  assert.equal(Object.hasOwn(preview.subscribers[0], "groupId"), false);
});
