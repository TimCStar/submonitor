...
function AppShell({ publicData, adminData, authenticated, initialPage, onRequireAdmin, onLogout, reloadAdmin }) {
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="sidebar-brand"><div className="brand-mark small"><Activity size={20} /></div><span>SubMonitor</span></div><nav>{NAVIGATION.filter((item) => authenticated || !item.admin).map((item) => { const Icon = item.icon; return <button key={item.id} className={page === item.id ? "active" : ""} onClick={() => navigate(item)}><Icon size={19} /><span>{item.label}</span></button>; })}</nav>{authenticated && <button className="logout-button" onClick={onLogout}><LogOut size={18} /><span>退出后台</span></button>}</aside>
