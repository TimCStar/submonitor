...
function LoginDialog({ open, onClose, onAuthenticated }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setTotp("");
      setError("");
      api.twoFactorPublicStatus().then((result) => setTwoFactorRequired(result.enabled)).catch(() => setTwoFactorRequired(false));
    }
  }, [open]);
  if (!open) return null;
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (requestError.code === "TWO_FACTOR_INVALID") setTotp("");
    ...
    <div className="dialog-backdrop" onMouseDown={onClose}>
      <section className="login-panel dialog-panel" role="dialog" aria-modal="true" aria-labelledby="admin-login" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <form onSubmit={submit}>
          ...
          <div className="input-with-icon"><KeyRound size={17} /><PasswordInput id="password" autoFocus required value={password} onChange={(event) => setPassword(event.target.value)} placeholder="" autoComplete="current-password" /></div>
          {twoFactorRequired && <><label htmlFor="totp">身份验证器验证码</label><div className="input-with-icon"><ShieldCheck size={17} /><input id="totp" type="text" inputMode="numeric" pattern="[0-9]{6}" maxLength="6" autoComplete="one-time-code" value={totp} onChange={(event) => setTotp(event.target.value.replace(/\D/g, "").slice(0, 6))} required /></div></>}
...
function Sparkline({ values }) {
  ...
  const points = values
    .map((value, index) => `${(index / (values.length - 1)) * 100},${96 - ((value - min) / range) * 90}`)
    .join(" ");
  return <svg className="sparkline" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><polyline points={points} fill="none" stroke="currentColor" strokeWidth="3" vectorEffect="non-scaling-stroke" /></svg>;
}

function QuotaPanel({ selector, snapshot, now, candidate, series }) {
...
function PasswordInput({ value, onChange, placeholder, autoComplete, ...rest }) {
  const [visible, setVisible] = useState(false);
  return (
    <div className="password-field">
      <input type={visible ? "text" : "password"} value={value} onChange={onChange} placeholder={placeholder} autoComplete={autoComplete} {...rest} />
      <button type="button" className="password-toggle" onClick={() => setVisible((current) => !current)} aria-label={visible ? "隐藏" : "显示"} tabIndex="-1">{visible ? <EyeOff size={15} /> : <Eye size={15} />}</button>
    </div>
  );
}
