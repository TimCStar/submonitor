...
function Overview({ data, selectedId, onSelectMonitor, onSelectEvent, now, onAdmin }) {
  ...
  return (
    <div className="page-stack">
      ...
      <section className="metrics-band"><div><span>运行模式</span><strong>{monitor.dryRun ? "预览" : "自动执行"}</strong></div><div><span>确认次数</span><strong>{monitor.confirmationsRequired} 次</strong></div><div><span>轮询间隔</span><strong>{monitor.pollIntervalSeconds} 秒</strong></div><div><span>并发</span><strong>{monitor.concurrency ? `${monitor.concurrency.current} / ${monitor.concurrency.max}` : "--"}</strong></div><div><span>目标账号</span><strong>{monitor.targetAccountCount}</strong></div><div><span>上次检查</span><strong>{formatDate(monitor.runtime.lastPollAt)}</strong></div></section>
...
function PublicSubscriberPreview({ monitor }) {
  ...
  const enabled = monitor.publicSubscriberPreviewEnabled !== false && monitor.subscriptionGroupMode !== "none";
...
function SavedBadge({ configured, invalid, onClear }) {
  if (!configured) return null;
  return <em className={invalid ? "invalid" : ""}>{invalid ? "需要重新输入" : "已保存"}{!invalid && onClear && <button type="button" className="text-button" onClick={onClear}>清除</button>}</em>;
}
