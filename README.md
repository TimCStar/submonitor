...
- 额度窗口使用率趋势图
- 使用率预警：超过配置阈值、额度耗尽两个事件分别通知一次（每个窗口周期各一次）
- `5% -> 0%`、`0% -> 0%` 均可识别
- 连续快照确认，避免瞬时数据误判
...
每个监控任务可以在「重置提醒」区域独立配置 Telegram、Bark 和邮件三个通知渠道。识别到自然重置并完成确认后立即推送（先通知、后执行动作），预览模式同样会通知。每类通知可通过对应渠道旁的「发送测试」按钮验证配置。

通知场景：

- 重置确认：使用率从旧周期切换到新周期（先通知、后执行动作）
- 使用率预警：使用率超过「额度监控」中的预警阈值（默认 80%）时提醒一次；额度耗尽（达到 100% 或上游 `limit_reached`）时再提醒一次。进入对应状态时各通知一次，使用率回落后再次触发
- 动作失败：`recover-state`、`reset-quota` 等写操作失败时提醒（失败动作会持续重试，同一事件每 2 小时最多提醒一次）
